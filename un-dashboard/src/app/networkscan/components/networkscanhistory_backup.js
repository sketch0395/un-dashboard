...
export default function NetworkScanHistory({ addZonesToTopology, scanHistoryData }) {
    ...
    const loadScanHistory = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const dbHistory = data.scanHistory || [];                // Convert database format to component format
                const convertedHistory = dbHistory.map(dbEntry => ({
                    ...
                    data: {}, // Leave empty - will be fetched on demand since main API excludes scanData
                    name: dbEntry.name,
                    settings: dbEntry.settings,
                    metadata: dbEntry.metadata,
                    _dbId: dbEntry._id, // Keep database ID for updates
                    isFromDatabase: true,
                    // Add scanSource for topology compatibility
                    scanSource: {
                        id: dbEntry.scanId,
                        name: dbEntry.name,
                        timestamp: format(new Date(dbEntry.createdAt), "yyyy-MM-dd HH:mm:ss")
    ...
    const saveScanToDatabase = async (scanEntry) => {
        try {
            ...
            console.log('Response headers:', [...response.headers.entries()]);            if (response.ok) {
                ...
                try {
                    ...
                    // Handle duplicate key errors that might come as 500 errors
    ...
    useEffect(() => {
        ...
        // Create a unique key for this scan to prevent race conditions
        const scanKey = `${ipRange}-${Date.now()}`;
        
        // Check if this scan is already being processed        if (scanSaveInProgress.has(scanKey)) {
            console.log("Scan save already in progress for:", scanKey);
            return;
        }
        
        // Mark this scan as being processed
        setScanSaveInProgress(prev => new Set([...prev, scanKey]));
        ...
        // Generate a unique scanId with timestamp to ensure uniqueness even in race conditions
        const scanId = `${uuidv4()}-${Date.now()}`;
        
        // Check for content-based duplicates in current history before proceeding
        const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
        const existingDuplicate = scanHistory.some(entry => {
        ...
        if (existingDuplicate) {
            console.log("Duplicate scan detected and prevented before state update:", { 
                deviceCount, 
                ipRange, 
                timestamp 
            });
          ...
          setScanHistory((prev) => {
            // Final scanId duplicate check (should never happen with timestamp-enhanced UUID)
            const scanIdExists = prev.some(entry => entry.id === scanId);
            if (scanIdExists) {
                console.log("Duplicate scanId detected (this should not happen):", scanId);
                return prev; // Don't add duplicate
            }

            const newEntry = {
                id: scanId, // Use the pre-generated scanId
                timestamp,
                ipRange,
                devices: deviceCount,
                data,
            ...
            // Clean up the scan save progress tracking
            setScanSaveInProgress(prev => {
    ...
    const syncToDatabase = async () => {
        ...
        try {
            ...
            setSyncError('Sync failed');        setIsSyncing(false);
            return false;
        }
    };
    ...
    const updateDeviceInHistory = async (deviceIP, updatedDeviceData) => {
        ...
        }    // Add a function to update scan data in history (for async data fetching)
    const updateScanData = (scanId, newData) => {
    ...
    const updateScanName = async (index, newName) => {
        ...
        if (scanToUpdate) {
            ...
            // If scan is from database, update in database too
            if (scanToUpdate.isFromDatabase && isAuthenticated && user) {
                try {
                    const response = await fetch(`/api/scan-history/${scanToUpdate.id}`, {
                        method: 'PUT',
                        credentials: 'include',
                    ...
                    if (response.ok) {
                        console.log(`Successfully updated scan name in database`);
                    } else {
                        console.error('Failed to update scan name in database:', response.status);
                    }
                } catch (error) {
    ...
    return (
        <ScanHistoryContext.Provider
            value={{ 
                ...
                updateScanData, // Add the new function
                clearScanHistoryOnLogout,
            ...
            if (!entry.data || Object.keys(entry.data).length === 0) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        // Update the entry with the full data
                        entry.data = fullScanData.scanData?.devices || fullScanData.scanData || {};                        console.log("Updated entry data:", typeof entry.data, Object.keys(entry.data));
